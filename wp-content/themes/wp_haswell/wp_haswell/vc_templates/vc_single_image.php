...
$a_class = ( $onclick != 'custom_link' ) ? 'cms-lightbox lightbox' : '';

$_vc_single_image = isset($vc_set_img_bg) ? $vc_set_img_bg : false;
$img_bg_url = wp_get_attachment_image_src( $img_id, 'full' );

$wrapp_class = ($_vc_single_image == true) ? 'wpb_wrapper cms-img-bg' : 'wpb_wrapper';
$wrap_equal_class = (isset($equal_height) && $equal_height == true) ? ' equal-height h-100 ' : '';
$none_hover_effect = (isset($vc_none_hover_effect) && $vc_none_hover_effect == true) ? ' none-hover-effect ' : '';

$img_output = ( 'vc_box_shadow_3d' === $style ) ? '<span class="vc_box_shadow_3d_wrap">' . $img['thumbnail'] . '</span>' : $img['thumbnail'];
if ( ! empty( $link_to ) ) {
	$image_string = '<a class="'.$a_class.'" href="' . esc_url($link_to) . '"' . ' target="' . $img_link_target . '"' . '><div class="vc_single_image-wrapper ' . $style . ' ' . $border_color . '">' . $img_output . '</div></a>';
} else {
	if ($_vc_single_image == true) {
		$image_string = '<div class="vc_single_image-wrapper cms-img-bg" style="background-image: url('.$img_bg_url[0].')"></div>';
	} else {
...
$output .= "\n\t\t" . '<div class="'.$wrapp_class.$wrap_equal_class.$none_hover_effect.'">';
