...
class WC_Widget_Cart_Search extends WP_Widget {
    public function __construct() {
        parent::__construct(
            ...
            __('Cart & Search', 'wp_haswell'), // Name
            array('description' => __("Display the user's Cart and Search form in the sidebar.", 'wp_haswell'),) // Args
        );
    ...
    function widget($args, $instance) {
        ...
        $search_id = rand(0, 999);
        ?>
        <div class="widget_cart_search_wrap">
            <div class="header">
                <?php if($show_search):?>
                    <a href="#cd-search<?php echo esc_attr($search_id); ?>" class="cd-search-trigger" data-display=".widget_searchform_content" data-no_display=".shopping_cart_dropdown" ><span></span></a>
                <?php endif; ?>
				<?php if($woocommerce && $show_cart):?>
                <a href="javascript:void(0)" class="icon_cart_wrap" data-display=".shopping_cart_dropdown" data-no_display=".widget_searchform_content"><i class="icon icon-ecommerce-bag-check"></i><span class="cart_total"><?php
            ...
            <div class="shopping_cart_dropdown">
                <div class="shopping_cart_dropdown_inner">
                    ...
                    <ul class="product-list-widget">
                        ...
                        <?php else : ?>
                            ...
                            <li class=".cms clearfix"><?php _e( 'Giỏ hàng trống', 'woocommerce' ); ?></li>
                ...
                <a href="<?php echo ''.$woocommerce->cart->get_cart_url(); ?>" class="cms-button gray-light left wc-forward"><?php _e( 'Cart', 'woocommerce' ); ?></a>
			...
			<?php endif;?>
			<?php if($show_search):?>
            <div id="cd-search<?php echo esc_attr($search_id); ?>" class="widget_searchform_content cd-search">
    ...
    function form( $instance ) {
        ...
        ?>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php _e( 'Title:', 'wp_haswell' ); ?></label>
            <input id="<?php echo esc_attr( $this->get_field_id('title') ); ?>" name="<?php echo esc_attr( $this->get_field_name('title') ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
        </p>
		<p>
            <label for="<?php echo esc_url($this->get_field_id('show_cart')); ?>"><?php _e( 'Show Cart:', 'wp_haswell' ); ?></label>
			<select id="<?php echo ''.$this->get_field_id('show_cart'); ?>" class="widefat" name="<?php echo ''.$this->get_field_name('show_cart'); ?>">
				<option value="0" <?php selected($show_cart,0); ?>><?php echo __('No','wp_haswell'); ?></option>
				<option value="1" <?php selected($show_cart,1); ?>><?php echo __('Yes','wp_haswell'); ?></option>
			</select>
        </p>
		<p>
            <label for="<?php echo esc_attr($this->get_field_id('show_search')); ?>"><?php _e( 'Show Search:', 'wp_haswell' ); ?></label>
			<select id="<?php echo ''.$this->get_field_id('show_search'); ?>" class="widefat" name="<?php echo ''.$this->get_field_name('show_search'); ?>">
				<option value="0" <?php selected($show_search,0); ?>><?php echo __('No','wp_haswell'); ?></option>
				<option value="1" <?php selected($show_search,1); ?>><?php echo __('Yes','wp_haswell'); ?></option>
			</select>
        </p>
    <?php
...
add_filter('woocommerce_add_to_cart_fragments', 'woocommerce_header_add_to_cart_content');
