...
function custom_nav_update($menu_id, $menu_item_db_id, $args ) {
    $fields = array('submenu_type','dropdown','widget_area','column_width','group','hide_link','bg_image','bg_image_attachment','bg_image_size','bg_image_position','bg_image_repeat','bg_color','icon_color','icon_font_size','menu_icon','el_class','custom_attr');
...
function start_el( &$output, $item, $depth = 0, $args = array(), $current_object_id = 0 ) {
    ...
    if ( ! empty( $item->_invalid ) ) {
        ...
        $title = sprintf( __( '%s (Invalid)', 'wp_haswell'), $item->title );
    } elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
        $classes[] = 'pending';
        /* translators: %s: title of menu item in draft status */
        $title = sprintf( __('%s (Pending)', 'wp_haswell'), $item->title );
    }
    ...
    <li data-menuanchor="" id="menu-item-<?php echo ''.$item_id; ?>" class="<?php echo implode(' ', $classes ); ?>">
            ...
            <dt class="menu-item-handle">
                ...
                <span class="item-controls">
                    ...
                    <span class="item-order hide-if-js">
                        ...
                        ?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up','wp_haswell'); ?>">&#8593;</abbr></a>
                        |
                        <a href="<?php
                            echo wp_nonce_url(
                                add_query_arg(
                        ...
                        ?>" class="item-move-down"><abbr title="<?php esc_attr_e('Move down','wp_haswell'); ?>">&#8595;</abbr></a>
                    </span>
                    <a class="item-edit" id="edit-<?php echo ''.$item_id; ?>" title="<?php esc_attr_e('Edit Menu Item','wp_haswell'); ?>" href="<?php
                        echo ( isset( $_GET['edit-menu-item'] ) && $item_id == $_GET['edit-menu-item'] ) ? admin_url( 'nav-menus.php' ) : add_query_arg( 'edit-menu-item', $item_id, remove_query_arg( $removed_args, admin_url( 'nav-menus.php#menu-item-settings-' . $item_id ) ) );
                    ?>"><?php _e( 'Edit Menu Item' ,'wp_haswell'); ?></a>
                </span>
            </dt>
        </dl>
        ...
        <div class="menu-item-settings" id="menu-item-settings-<?php echo ''.$item_id; ?>">
            ...
            <p class="description description-thin">
                <label for="edit-menu-item-title-<?php echo ''.$item_id; ?>">
                    <?php _e( 'Navigation Label' ,'wp_haswell'); ?><br />
                    <input type="text" id="edit-menu-item-title-<?php echo ''.$item_id; ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo ''.$item_id; ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
                </label>
            </p>
            <p class="description description-thin">
                <label for="edit-menu-item-attr-title-<?php echo ''.$item_id; ?>">
                    <?php _e( 'Title Attribute','wp_haswell' ); ?><br />
                    <input type="text" id="edit-menu-item-attr-title-<?php echo ''.$item_id; ?>" class="widefat edit-menu-item-attr-title" name="menu-item-attr-title[<?php echo ''.$item_id; ?>]" value="<?php echo esc_attr( $item->post_excerpt ); ?>" />
                </label>
            </p>
            <p class="description description-thin">
                <label for="edit-menu-item-attr-custom-<?php echo ''.$item_id; ?>">
                    <?php _e( 'Data Group','wp_haswell' ); ?><br />
                    <input type="text" id="edit-menu-item-custom_attr-<?php echo ''.$item_id; ?>" class="widefat edit-menu-item-attr-custom" name="menu-item-custom_attr[<?php echo ''.$item_id; ?>]" value="<?php echo esc_attr( $item->custom_attr ); ?>" />
                </label>
            </p>
            <p class="field-link-target description">
                <label for="edit-menu-item-target-<?php echo ''.$item_id; ?>">
                    <input type="checkbox" id="edit-menu-item-target-<?php echo ''.$item_id; ?>" value="_blank" name="menu-item-target[<?php echo ''.$item_id; ?>]"<?php checked( $item->target, '_blank' ); ?> />
                    <?php _e( 'Open link in a new window/tab' ,'wp_haswell'); ?>
                </label>
            </p>
            <p class="field-css-classes description description-thin">
                <label for="edit-menu-item-classes-<?php echo ''.$item_id; ?>">
                    <?php _e( 'CSS Classes (optional)' ,'wp_haswell'); ?><br />
                    <input type="text" id="edit-menu-item-classes-<?php echo ''.$item_id; ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo ''.$item_id; ?>]" value="<?php echo esc_attr( implode(' ', $item->classes ) ); ?>" />
                </label>
            </p>
            <p class="field-xfn description description-thin">
                <label for="edit-menu-item-xfn-<?php echo ''.$item_id; ?>">
                    <?php _e( 'Link Relationship (XFN)' ,'wp_haswell'); ?><br />
                    <input type="text" id="edit-menu-item-xfn-<?php echo ''.$item_id; ?>" class="widefat code edit-menu-item-xfn" name="menu-item-xfn[<?php echo ''.$item_id; ?>]" value="<?php echo esc_attr( $item->xfn ); ?>" />
                </label>
            </p>
            <p class="field-description description description-wide">
                <label for="edit-menu-item-description-<?php echo ''.$item_id; ?>">
                    <?php _e( 'Description' ,'wp_haswell'); ?><br />
                    ...
                    <span class="description"><?php _e('The description will be displayed in the menu if the current theme supports it.','wp_haswell'); ?></span>
                </label>
            </p>
            <?php
            /*
            ...
            <p class="description description-wide description_width_100">
                ...
                <label for="edit-<?php echo ''.$key . '-' . $item_id; ?>">
                    <select id="edit-<?php echo ''.$key . '-' . $item_id; ?>" class=" <?php echo ''.$key; ?>" name="<?php echo ''.$key . "[" . $item_id . "]"; ?>">
                        <option value="standard" <?php echo ( $value == 'standard' ) ? ' selected="selected" ' : ''; ?>><?php _e( 'Standard Dropdown', 'wp_haswell' ); ?></option>
                        <option value="columns2" <?php echo ( $value == 'columns2' ) ? ' selected="selected" ' : ''; ?>><?php _e( '2 columns dropdown', 'wp_haswell' ); ?></option>
                        <option value="columns3" <?php echo ( $value == 'columns3' ) ? ' selected="selected" ' : ''; ?>><?php _e( '3 columns dropdown', 'wp_haswell' ); ?>
                        </option>
                        <option value="columns4" <?php echo ( $value == 'columns4' ) ? ' selected="selected" ' : ''; ?>><?php _e( '4 columns dropdown', 'wp_haswell' ); ?></option>
                        <option value="columns5" <?php echo ( $value == 'columns5' ) ? ' selected="selected" ' : ''; ?>><?php _e( '5 columns dropdown', 'wp_haswell' ); ?></option>
                    </select>
                </label>
            </p>
            <?php
            ...
            <p class="description description-wide description_width_100">
                ...
                <label for="edit-<?php echo ''.$key . '-' . $item_id; ?>">
                    <select id="edit-<?php echo ''.$key . '-' . $item_id; ?>" class=" <?php echo ''.$key; ?>" name="<?php echo ''.$key . "[" . $item_id . "]"; ?>">
                        <option value="autodrop_submenu" <?php echo ( $value == 'autodrop_submenu' ) ? ' selected="selected" ' : ''; ?>><?php _e( 'Auto drop', 'wp_haswell' ); ?></option>
                        <option value="drop_to_left" <?php echo ( $value == 'drop_to_left' ) ? ' selected="selected" ' : ''; ?>><?php _e( 'Drop To Left Side', 'wp_haswell' ); ?></option>
                        <option value="drop_to_right" <?php echo ( $value == 'drop_to_right' ) ? ' selected="selected" ' : ''; ?>><?php _e( 'Drop To Right Side', 'wp_haswell' ); ?></option>
                        <option value="drop_to_center" <?php echo ( $value == 'drop_to_center' ) ? ' selected="selected" ' : ''; ?>><?php _e( 'Drop To Center', 'wp_haswell' ); ?></option>
                        <option value="drop_full_width" <?php echo ( $value == 'drop_full_width' ) ? ' selected="selected" ' : ''; ?>><?php _e( 'Full width', 'wp_haswell' ); ?></option>
                    </select>
                </label>
            </p>
            <?php
            ...
            <p class="description description-wide description_width_100 el_widget_area">
                ...
                <label for="edit-<?php echo ''.$key . '-' . $item_id; ?>">
                    <select id="edit-<?php echo ''.$key . '-' . $item_id; ?>" class=" <?php echo ''.$key; ?>" name="<?php echo ''.$key . "[" . $item_id . "]"; ?>">
                        <option value="" <?php echo ( $value == '' ) ? ' selected="selected" ' : ''; ?>><?php _e( 'Select Widget Area', 'wp_haswell' ); ?></option>
                        <?php
            ...
            <p class="description description-wide description_width_100">
                ...
                <label for="edit-<?php echo ''.$key . '-' . $item_id; ?>">
                    <select id="edit-<?php echo ''.$key . '-' . $item_id; ?>" class=" <?php echo ''.$key; ?>" name="<?php echo ''.$key . "[" . $item_id . "]"; ?>">
                        <option value="no_group" <?php echo ( $value == 'no_group' ) ? ' selected="selected" ' : ''; ?>><?php _e( 'No', 'wp_haswell' ); ?></option>
                        <option value="group" <?php echo ( $value == 'group' ) ? ' selected="selected" ' : ''; ?>><?php _e( 'Yes', 'wp_haswell' ); ?></option>
                    </select>
                </label>
            </p>
            <?php
            ...
            <p class="description description-wide description_width_100">
                ...
                <label for="edit-<?php echo ''.$key . '-' . $item_id; ?>">
                    <select id="edit-<?php echo ''.$key . '-' . $item_id; ?>" class=" <?php echo ''.$key; ?>" name="<?php echo ''.$key . "[" . $item_id . "]"; ?>">
                        <option value="0" <?php echo ( $value == '0' ) ? ' selected="selected" ' : ''; ?>><?php _e( 'No', 'wp_haswell' ); ?></option>
                        <option value="1" <?php echo ( $value == '1' ) ? ' selected="selected" ' : ''; ?>><?php _e( 'Yes', 'wp_haswell' ); ?></option>
                    </select>
                </label>
            </p>
        <?php }
        ...
        <p class="description description-wide obtheme_checkbox obtheme_mega_menu obtheme_mega_menu_d1">
            <label for="edit-<?php echo ''.$key . '-' . $item_id; ?>">
                <?php echo esc_html( $title ); ?><br />
                <input type="text" value="<?php echo ''.$value; ?>" id="edit-<?php echo ''.$key . '-' . $item_id; ?>" class=" <?php echo ''.$key; ?>" name="<?php echo ''.$key . "[" . $item_id . "]"; ?>" />
                <input alt="#TB_inline?height=400&width=500&inlineId=<?php echo ''.$key . '-' . $item_id . '-popup'; ?>" title="<?php _e( 'Click to browse icon', 'wp_haswell' ) ?>" class="thickbox button-secondary submit-add-to-menu" type="button" value="<?php _e( 'Browse Icon', 'wp_haswell' ) ?>" />
            ...
            <p class="description description-wide obtheme_checkbox obtheme_mega_menu obtheme_mega_menu_d2">
                <label for="edit-<?php echo ''.$key . '-' . $item_id; ?>">
                    <span class='obtheme_long_desc'><?php echo esc_html( $title ); ?></span><br />
                    <input type="text" value="<?php echo ''.$value; ?>" id="edit-<?php echo ''.$key . '-' . $item_id; ?>" class=" <?php echo ''.$key; ?>" name="<?php echo ''.$key . "[" . $item_id . "]"; ?>" />
                    <button id="browse-edit-<?php echo ''.$key . '-' . $item_id; ?>" class="set_custom_images button button-secondary submit-add-to-menu"><?php _e( 'Browse Image', 'wp_haswell' ); ?></button>
                    <a class="button btn_clear button-primary" href="javascript: void(0);">Clear</a>
                </label>
            </p>
            ...
            <div class="menu-item-actions description-wide submitbox">
                <?php if( 'custom' != $item->type && $original_title !== false ) : ?>
                    <p class="link-to-original">
                        <?php printf( __('Original: %s', 'wp_haswell'), '<a href="' . esc_url( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
                    </p>
                <?php endif; ?>
                ...
                ); ?>"><?php _e('Remove','wp_haswell'); ?></a> <span class="meta-sep"> | </span> <a class="item-cancel submitcancel" id="cancel-<?php echo ''.$item_id; ?>" href="<?php echo esc_url( add_query_arg( array('edit-menu-item' => $item_id, 'cancel' => time()), remove_query_arg( $removed_args, admin_url( 'nav-menus.php' ) ) ) );
                    ?>#menu-item-settings-<?php echo ''.$item_id; ?>"><?php _e('Cancel','wp_haswell'); ?></a>
            </div>
...
class HeroMenuWalker extends Walker_Nav_Menu {
    ...
    public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        $atts['data-group']   = ! empty( $item->custom_attr )        ? $item->custom_attr        : '';
