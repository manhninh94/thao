...
function cms_options($params = array())
{
    ...
    if (is_admin() && !empty($params['id']) && isset($core_options)) {
        ...
        elseif ($pagenow=='post-new.php' || $pagenow=='post.php'){
            ...
            // Get value
            if(!empty($haswell_meta->$params['id'])){
                $params['value'] = $haswell_meta->$params['id'];
            } else {
                $params['value'] = null;
            }
...
class CsCoreControl
{
    ...
    private function select($params)
    {
        ...
        <select name="<?php echo esc_attr($params['id']); ?>" id="<?php echo esc_attr($params['id']); ?>"<?php echo $data; ?>>
        ...
        <div class="images-field clearfix">
            ...
            foreach ($selected as $value):
                ...
                <li class="items" data-id="<?php echo esc_attr($value); ?>" style="background-image:url(<?php echo esc_url($attachment_image[0]);?>);background-size:cover;">
                    <i class="edit dashicons dashicons-plus-alt" title="<?php _e('Replace Image', 'cms-theme-framework'); ?>"></i>
                    <i class="remove dashicons dashicons-dismiss" title="<?php _e('Remove Image', 'cms-theme-framework'); ?>"></i>
                </li>
            <?php endif; endforeach; ?>
            <?php if($params['field'] != 'single'): ?>
                <li class="items" data-id=""><i class="add dashicons dashicons-plus-alt" title="<?php _e('Add Image', 'cms-theme-framework'); ?>"></i></li>
		...
		foreach($_POST as $key => $value) {
			if(strstr($key, '_cms_')) {
			    ...
			    if($key === '_cms_single_portf_media'){
			       ...
			       $haswell_meta[$key] = $value;
			    } else {
			    
			       $haswell_meta[$key] = rawurlencode($value);
			    }
			}
		}
		/* update _cms_meta_data. */
		if(!empty($haswell_meta)){
		  update_post_meta($post_id, '_cms_meta_data', json_encode($haswell_meta));
		}
	}
}