	...
	public function widget( $args, $instance ) {
		...
		echo '<form method="get" action="' . esc_url( $form_action ) . '">
			<div class="cms-filter-price-wrap">
				<div class="row mb-20">
					<div class="col-xs-6">
						<input type="text" class="form-control" name="min_price" value="' . esc_attr( $min_price ) . '" placeholder="' . esc_attr__('FROM', 'woocommerce' ) . '" maxlength="100"/>
					</div>
					<div class="col-xs-6">
						<input type="text" class="form-control" name="max_price" value="' . esc_attr( $max_price ) . '" placeholder="' . esc_attr__( 'TO', 'woocommerce' ) . '" maxlength="100" />
					</div>
				</div>
				<button type="submit" class="cms-button medium gray-light btn-block">' . __( 'Filter', 'woocommerce' ) . '</button>
				' . $fields . '
			</div>
		</form>';
